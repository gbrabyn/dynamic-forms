...
class FormAbstractTest extends PHPUnit_Framework_TestCase 
{
    ...
    public function testApplyToChildren()
    {
        $escaper = new GBrabyn\DynamicForms\Element\LaminasEscaperWrapper( new Laminas\Escaper\Escaper('utf-8') );
        $form = $this->getClass();
        $form->addUniveralTransformers(new GBrabyn\DynamicForms\Transform\Trim);
        $form->registerElement('inputText', new \GBrabyn\DynamicForms\Element\html5\Input($escaper, 'text'));
        $form->setDefaultErrorDecorator(new \GBrabyn\DynamicForms\ErrorDecorator\html5\RightSide());
        ...
        $form->applyToChildren(
                    'ddd', 
                    $formValues, 
                    [new GBrabyn\DynamicForms\Transform\RemoveNumberLocalisation('de_DE')],
                    [new GBrabyn\DynamicForms\FieldValidator\Required()]
                );
        
        $form->populate($formValues, true);
        $valid = $form->validate();

        $this->assertFalse($valid);
        
        $el1 = $form->element('inputText', 'ddd[]', 'ddd.0');
        $expected1 = '<input type="text" name="ddd[]" value="1222.34" >';
        
        $this->assertEquals($expected1, $el1);
        
        $el2 = $form->element('inputText', 'ddd[]', 'ddd.1');
        $expected2 = '<input type="text" name="ddd[]" value="1.55" >';
        
        $this->assertEquals($expected2, $el2);
        
        $el3 = $form->element('inputText', 'ddd[]', 'ddd.2');
        $expected3 = '<input type="text" name="ddd[]" value="" class="error"><span class="error msgRight">Required</span>';
        
        $this->assertEquals($expected3, $el3);
        
        $this->assertEquals('9.123,12', $form->field('eee.0')->getValue());
        $this->assertEquals('10.123,26', $form->field('fff')->getValue());

        $form2->applyToChildren(
            'ddd',
            [],
            [new GBrabyn\DynamicForms\Transform\RemoveNumberLocalisation('de_DE')],
            [new GBrabyn\DynamicForms\FieldValidator\Required()]
        ...
        $form2->populate([], true);
        $valid2 = $form2->validate();
        $this->assertTrue($valid2);
        $this->assertEquals([], $form2->field('ddd')->getValue());
        $this->assertEquals(['ddd'=>[]], $form2->getDefinedValues());
    }
